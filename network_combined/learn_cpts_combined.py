...
def create_network_structure():
    """
    Create Bayesian network structure with 21 genes mapped to 13 pathway nodes,
    plus TissueType and DrugResponse. Returns (structure, gene_nodes).
    """
    ...
    gene_nodes = [
        ...
        # Histone / Epigenetic / Transcription (7)
        'KAT6A_amp', 'TBL1XR1_amp', 'RUNX1_amp', 'MYC_amp',
        'CREBBP_alt', 'EP300_alt', 'HDAC1_alt',

        # DNA Damage / Repair (3)
        'TP53_mut', 'BRCA1_mut', 'BRCA2_mut',  # TP53 also participates across groups

        # Additional DNA/Growth (5)
        'CHEK1_mut', 'RAD51C_mut', 'EGFR_alt', 'ERBB2_alt', 'PIK3CA_mut'
    ]

    # 13 pathway nodes

    pathway_nodes = [
        # From graph fragments & domain semantics
        'CellCycleControl',          # CDK-related / RB activity
        'CDK_Overdrive',             # overdrive / hyperactivation of CDKs
        'RB_Pathway_Activity',       # RB-specific pathway
        'Chromatin_Remodeling_State',# chromatin state
        'Histone_Transcription',     # histone acetylation/transcriptional control
        'Epigenetic_Dysregulation',  # epigenetic modifiers
        'DNA_Repair_Capacity',       # repair machinery capacity
        'DNA_Damage_Response',       # damage signaling
        'Genomic_Instability',       # aggregate instability readout
        'RTK_PI3K_Signaling',        # receptor-tyrosine kinase / PI3K axis
        'GrowthFactor_Signaling',    # growth signaling (overlap with RTK/PI3K)
        'Proliferative_Phenotype',   # higher-level proliferative phenotype      
        'DrugResponse_Modulator'     # intermediate module before DrugResponse (optional)
    ]

    other_nodes = ['DrugResponse']

    # ---------------------
    # Build structure: node -> list(parents)
    # ---------------------
    structure = {}

    # Genes: root nodes (no parents)
    for g in gene_nodes:
        structure[g] = []

    # # TissueType: root contextual node (we keep this separate from TissueContextModule)
    # structure['TissueType'] = []

    # # TissueContextModule (a pathway that summarizes TissueType; keep simple)
    # structure['TissueContextModule'] = ['TissueType']


    structure['CellCycleControl'] = [
        'RB1_mut', 'CCND1_amp', 'CDK4_mut', 'CDK6_mut', 'CDKN2A_del', 'CDKN2B_del',
    ]

    structure['CDK_Overdrive'] = [
        'CCND1_amp', 'CDK4_mut', 'CDK6_mut', 'MYC_amp', 
    ]

    structure['RB_Pathway_Activity'] = [
        'RB1_mut', 'CDKN2A_del', 'CDKN2B_del', 
    ]

    structure['Chromatin_Remodeling_State'] = [
        'KAT6A_amp', 'TBL1XR1_amp', 'RUNX1_amp', 'CREBBP_alt', 'EP300_alt', 'HDAC1_alt',
    ]

    structure['Histone_Transcription'] = [
        'Chromatin_Remodeling_State', 'MYC_amp', 'TP53_mut',
    ]

    structure['Epigenetic_Dysregulation'] = [
        'CREBBP_alt', 'EP300_alt', 'HDAC1_alt', 'KAT6A_amp', 'TBL1XR1_amp',
    ]

    structure['DNA_Repair_Capacity'] = [
        'BRCA1_mut', 'BRCA2_mut', 'RAD51C_mut', 'CHEK1_mut',
    ]

    structure['DNA_Damage_Response'] = [
        'TP53_mut', 'CHEK1_mut', 'RAD51C_mut', 'DNA_Repair_Capacity', 
    ]

    structure['Genomic_Instability'] = [
        'BRCA1_mut', 'BRCA2_mut', 'TP53_mut', 'RAD51C_mut', 'DNA_Repair_Capacity'
    ]

    structure['RTK_PI3K_Signaling'] = [
        'EGFR_alt', 'ERBB2_alt', 'PIK3CA_mut', 
    ]

    structure['GrowthFactor_Signaling'] = [
        'RTK_PI3K_Signaling', 'MYC_amp', 'PIK3CA_mut',
    ]

    structure['Proliferative_Phenotype'] = [
        'CellCycleControl', 'CDK_Overdrive', 'RB_Pathway_Activity', 'GrowthFactor_Signaling',
        'Histone_Transcription'
    ]

    structure['DrugResponse_Modulator'] = [
        'Proliferative_Phenotype', 'Genomic_Instability', 'Epigenetic_Dysregulation', 
    ]

    # Final sink: DrugResponse depends on all pathway modules (we include the modulator and major pathways)
    structure['DrugResponse'] = [
        'CellCycleControl', 'CDK_Overdrive', 'RB_Pathway_Activity',
        'Chromatin_Remodeling_State', 'Histone_Transcription', 'Epigenetic_Dysregulation',
        'DNA_Repair_Capacity', 'DNA_Damage_Response', 'Genomic_Instability',
        'RTK_PI3K_Signaling', 'GrowthFactor_Signaling', 'Proliferative_Phenotype',
        'DrugResponse_Modulator',
    ]

    # Return structure and node lists
...
def create_data_matrix(mutations, amplifications, deletions, drug_response, 
    ...
    # Cell Cycle Response: OR of Cell Cycle genes (no pathway dependencies)
    cell_cycle_genes = ['RB1_mut', 'CCND1_amp', 'CDK4_mut', 'CDK6_mut', 'CDKN2A_del', 'CDKN2B_del',]
    cell_cycle_cols = [node_to_col[g] for g in cell_cycle_genes if g in node_to_col]
    if cell_cycle_cols:
        node_to_col['CellCycleControl'] = col_idx
        gene_data = np.column_stack([gene_data, np.max(gene_data[:, cell_cycle_cols], axis=1)])
        col_idx += 1
    ...
    chromatin_genes = ["CREBBP_alt", "EP300_alt", "HDAC1_alt", "KAT6A_amp", "TBL1XR1_amp"]
    ...
    histone_genes = ['MYC_amp', 'TP53_mut']
    histone_cols = [node_to_col[g] for g in histone_genes if g in node_to_col]
    if histone_cols and 'Chromatin_Remodeling_State' in node_to_col:
        gene_contrib = np.max(gene_data[:, histone_cols], axis=1)
        dna_contrib = gene_data[:, node_to_col['Chromatin_Remodeling_State']]
    ...
    #'CREBBP_alt', 'EP300_alt', 'HDAC1_alt', 'KAT6A_amp', 'TBL1XR1_amp',
    epigenetic_genes = ['CREBBP_alt', 'EP300_alt', 'HDAC1_alt', 'KAT6A_amp', 'TBL1XR1_amp']
    epigenetic_cols = [node_to_col[g] for g in epigenetic_genes if g in node_to_col]
    if epigenetic_cols:
        node_to_col['Epigenetic_Dysregulation'] = col_idx
        gene_data = np.column_stack([gene_data, np.max(gene_data[:, epigenetic_cols], axis=1)])
        col_idx += 1
        
    # DNA Repair Capacity: OR of DNA repairing genes (no pathway dependencies)
    dna_genes = ["BRCA1_mut","BRCA2_mut","CHEK1_mut","RAD51C_mut"]
    ...
    #DNA Damage Response
    #  'TP53_mut', 'CHEK1_mut', 'RAD51C_mut'
    dna_damage_genes = ['TP53_mut', 'CHEK1_mut', 'RAD51C_mut']
    dna_damage_cols = [node_to_col[g] for g in dna_damage_genes if g in node_to_col]
    if dna_damage_cols and 'DNA_Repair_Capacity' in node_to_col:
        # Combine gene contributions with DNA_Damage_Response
        gene_contrib = np.max(gene_data[:, dna_damage_cols], axis=1)
        dna_contrib = gene_data[:, node_to_col['DNA_Repair_Capacity']]
        node_to_col['DNA_Damage_Response'] = col_idx
        gene_data = np.column_stack([gene_data, np.maximum(gene_contrib, dna_contrib)])
        col_idx += 1
        
    #Genomic Instability 'BRCA1_mut', 'BRCA2_mut', 'TP53_mut', 'RAD51C_mut'
    # DrugResponse: Binarize based on median (handle missing values)
    genomic_genes = ['BRCA1_mut', 'BRCA2_mut', 'TP53_mut', 'RAD51C_mut']
    genomic_cols = [node_to_col[g] for g in genomic_genes if g in node_to_col]
    if genomic_cols and 'DNA_Repair_Capacity' in node_to_col:
        # Combine gene contributions with DNA_Damage_Response
        gene_contrib = np.max(gene_data[:, genomic_cols], axis=1)
        dna_contrib = gene_data[:, node_to_col['DNA_Repair_Capacity']]
        node_to_col['Genomic_Instability'] = col_idx
        gene_data = np.column_stack([gene_data, np.maximum(gene_contrib, dna_contrib)])
        col_idx += 1
        
    # RTK PI3K Signaling: OR of RTK PI3K Signaling genes (no pathway dependencies)
    rtk_genes = ['EGFR_alt', 'ERBB2_alt', 'PIK3CA_mut']
    ...
    #Growth Factor 'RTK_PI3K_Signaling', 'MYC_amp', 'PIK3CA_mut'
    growth_factor_genes = ['MYC_amp', 'PIK3CA_mut']
    growth_factor_cols = [node_to_col[g] for g in growth_factor_genes if g in node_to_col]
    if growth_factor_cols and 'RTK_PI3K_Signaling' in node_to_col:
        gene_contrib = np.max(gene_data[:, growth_factor_cols], axis=1)
        rtk_contrib = gene_data[:, node_to_col['RTK_PI3K_Signaling']]
        node_to_col['GrowthFactor_Signaling'] = col_idx
        gene_data = np.column_stack([gene_data, np.maximum(gene_contrib, rtk_contrib)])
        col_idx += 1
        
    
        
    #Proliferative Phenotype 'CellCycleControl', 'CDK_Overdrive', 'RB_Pathway_Activity', 'GrowthFactor_Signaling' 'Histone_Transcription'
    pathway_contribs = []
    for parent in [
        'CellCycleControl',
        'CDK_Overdrive',
        ...
        if parent in node_to_col:
            pathway_contribs.append(gene_data[:, node_to_col[parent]])

    if pathway_contribs:
        # OR across all parent pathways
        phenotype_values = np.max(np.column_stack(pathway_contribs), axis=1)

        node_to_col['Proliferative_Phenotype'] = col_idx
        gene_data = np.column_stack([gene_data, phenotype_values])

        col_idx += 1
        
    #Drug Response Modulator 'Proliferative_Phenotype', 'Genomic_Instability', 'Epigenetic_Dysregulation'
    pathway_contribs = []
    ...
    if pathway_contribs:
        ...
        node_to_col['DrugResponse_Modulator'] = col_idx
        gene_data = np.column_stack([gene_data, phenotype_values])
...
def main():
    ...
    pathway_nodes = ['CellCycleControl',
    'CDK_Overdrive',
    'RB_Pathway_Activity',
    'Chromatin_Remodeling_State',
    'Histone_Transcription',
